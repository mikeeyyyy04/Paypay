    ...
    <section className="page">
      <header className="page-header dashboard-hero">
        ...
        <div className="hero-summary">
          ...
          <small>{nextOrder ? `${nextOrder.customer} - USD ${nextOrder.amount.toFixed(2)}` : 'No pending transfers'}</small>
        </div>
      </header>
      ...
      <div className="stats-grid">
        ...
        <StatCard label="Paid revenue" value={`USD ${stats.revenue.toFixed(2)}`} helper={`${paidRevenueRate}% of submitted value`} />
      </div>
        ...
        <div className="activity-grid">
          {recentOrders.map((order) => (
            <article className="activity-item activity-order" key={order.id}>
              ...
              <p>{order.customer}</p>
              <small>
                {order.classTitle} - USD {order.amount.toFixed(2)}
              </small>
            </article>
          ))}
        </div>
